...
class BPTTIterator(Iterator):
    ...
    def __iter__(self):
        ...
        # copy pasta for tags
        tags = [tag for ex in raw_data for tag in ex.tags]
        tags = tags + (
            [TAGS.pad_token]
            * int(
                math.ceil(len(tags) / self.batch_size)
                * self.batch_size
                - len(tags)
            )
        )
        tag_data = TAGS.numericalize(
            [tags], device=self.device)
        tag_data = tag_data.view(self.batch_size, -1).t()
        eos_token = TAGS.vocab["<eos>"]
        tag_data = th.cat([
            th.empty(
                self.batch_size,
                device=data.device,
